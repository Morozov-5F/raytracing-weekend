...
rt_texture_image_t *rt_texture_image_new(const char *filename)
{
    ...
    rt_texture_init(&result->base, RT_TEXTURE_TYPE_IMAGE);
...
{
    assert(NULL != texture_image);

    if (NULL == texture_image->image_data)
    {
        return colour(0,1,1);
    }
    ...
    int i = (int)(u * texture_image->width);
    int j = (int)(v * texture_image->height);

    i = i >= texture_image->width ? texture_image->width - 1 : i;
    j = j >= texture_image->height ? texture_image->width - 1 : j;

    unsigned char *pixel = texture_image->image_data + i * 3 + j * texture_image->bytes_per_scanline;
