...
rt_instance_t *rt_instance_new(rt_hittable_t *hittable)
{
    ...
    rt_hittable_init(&result->base, RT_HITTABLE_TYPE_INSTANCE);
...
void rt_instance_rotate_y(rt_instance_t *instance, double y)
{
    ...
    instance->transform_matrix_ray = rt_matrix_rotation_y(-RT_DEG_TO_RAD(y));
    instance->transform_matrix_bb = rt_matrix_rotation_y(RT_DEG_TO_RAD(y));
}
...
bool rt_instance_hit(const rt_instance_t *instance, const ray_t *ray, double t_min, double t_max,
